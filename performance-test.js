...
async function insertMultipleCSVFiles(csvFiles) {
    ...
    try {
        ...
        for (let i = 0; i < csvFiles.length; i++) {
            ...
            // Use batch insert
            try {
                const inserted = await mysqlOps.batchInsertFromCSV(
                    csvFile,
                    "Libro",
                    fieldNames,
                    transform
                ...
                if ((i + 1) % 10 === 0 || i === csvFiles.length - 1) {
                    console.log(
                        `Processed ${i + 1} of ${
                            csvFiles.length
                        } CSV files, total rows inserted: ${totalInserted}`
                    );
                }
            } catch (err) {
    ...
    try {
        ...
        const transform = (rowData, headers) => {
            ...
            fieldNames.forEach((field, idx) => {
                ...
                if (
                    (field === "secondLastName" || field === "year") &&
                    (!value || value === '""')
                ) {
                    values.push(null);
                } else {
...
async function exportTablesToCSV() {
    ...
    try {
        ...
        const booksStream = fs.createWriteStream(exportedBooksCSV);
        booksStream.write(
            "id,ISBN,title,autor_license,editorial,pages,year,genre,language,format,sinopsis,content\n"
        );
        ...
        fs.writeFileSync(
            ...
            "id,license,name,lastName,secondLastName,year\n" +
                authors
                    .map((author) => {
                        return `${author.id},"${author.license}","${
                            author.name
                        }","${author.lastName || ""}","${
                            author.secondLastName || ""
                        }",${author.year || ""}`;
        ...
        console.log(
            `Exported ${books.length} books and ${authors.length} authors to CSV`
        );
    } catch (error) {
...
async function migrateAndRestore() {
    ...
    try {
        ...
        console.log(
            `Executing MongoDB export for books: mongoexport ${mongoExportBooks.ProcessArguments.join(
                " "
            )}`
        );
        ...
        console.log(
            `Executing MongoDB export for authors: mongoexport ${mongoExportAuthors.ProcessArguments.join(
                " "
            )}`
        );
        ...
        if (fs.existsSync(authorsJsonPath) && fs.existsSync(booksJsonPath)) {
            console.log(
                `MongoDB export successful. Files created: ${authorsJsonPath}, ${booksJsonPath}`
            );
        } else {
            console.error(
                `MongoDB export may have failed. Files not found: ${
                    !fs.existsSync(authorsJsonPath) ? authorsJsonPath : ""
                } ${!fs.existsSync(booksJsonPath) ? booksJsonPath : ""}`
            );
        }
