...
async function setupSchema() {
    ...
    `);
        ...
        console.log(
            "   You can run 'node setup-credentials.js' to configure your database connections"
        );
        console.log("   Make sure your MySQL user has sufficient privileges");
        console.log(
            "   You can grant privileges with: GRANT ALL PRIVILEGES ON *.* TO 'user'@'localhost';"
        );
    } finally {
        if (connection) {
...
function updateEnvFile() {
    ...
    try {
        ...
        if (envContent.includes("MONGO_URI=")) {
            ...
            const mongoUriMatch = envContent.match(
                /MONGO_URI=mongodb:\/\/([^@]+@)?([^\/]+)\/([^\/\s]+)/
            );
            if (mongoUriMatch) {
